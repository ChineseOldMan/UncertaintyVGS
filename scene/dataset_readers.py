...
def readMast3rSyntheticInfo(path, images, eval, n_views=0, llffhold=8):
    # ply_path = os.path.join(path, "sparse/0/points3D.ply")
    # bin_path = os.path.join(path, "sparse/0/points3D.bin")
    ply_path = os.path.join(path, str(n_views) + "_views/dense/fused.ply")
    str_n_views = str(n_views) + "_views"

    try:
        cameras_intrinsic_file = os.path.join(path, str_n_views,"mast3r", "cameras.bin")
        cameras_extrinsic_file = os.path.join(path, str_n_views,"mast3r",  "images.bin")
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_file)
        cam_intrinsics = read_intrinsics_binary(cameras_intrinsic_file)
    except:
        cameras_extrinsic_file = os.path.join(path, str_n_views,"mast3r",  "images.txt")
        cameras_intrinsic_file = os.path.join(path, str_n_views,"mast3r",  "cameras.txt")
        cam_extrinsics = read_extrinsics_text(cameras_extrinsic_file)
        cam_intrinsics = read_intrinsics_text(cameras_intrinsic_file)
...
sceneLoadTypeCallbacks = {
    ...
    "Mast3r" : readMast3rSyntheticInfo
}
