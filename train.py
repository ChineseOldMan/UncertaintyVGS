...
def training(dataset, opt, pipe, args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # print(f"Shape of midas_depth: {midas_depth.shape}")
        # print(f"Shape of rendered_depth: {rendered_depth.shape}")
        
        depth_loss = min(
                        (1 - pearson_corrcoef( - midas_depth.flatten(), rendered_depth.flatten())).mean(),
                        (1 - pearson_corrcoef((1 / (midas_depth + 200.)).flatten(), rendered_depth.flatten()))
        )
        ...
        if iteration % args.sample_pseudo_interval == 0 and iteration > args.start_sample_pseudo and iteration < args.end_sample_pseudo:
            ...
            depth_loss_pseudo = (1 - pearson_corrcoef(rendered_depth_pseudo.flatten(), -midas_depth_pseudo.flatten())).mean()
...
if __name__ == "__main__":
    ...
    torch.autograd.set_detect_anomaly(args.detect_anomaly) # 作用是检测异常
